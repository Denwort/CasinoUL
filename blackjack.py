...
def bj_suma_cartas(cartas):
    if len(cartas) != 0:
        suma = 0
        cantidad_de_As = 0
        for i in range(len(cartas)):
            carta = cartas[i]
            numero = numero_de_carta(carta)
...
def bj_tiene_blackjack(cartas):
    bj = False
    if len(cartas) == 2 and bj_suma_cartas(cartas) == 21:
        bj = True
    return bj 

def bj_puede_doblar(cartas):
    puede = False
    s = bj_suma_cartas(cartas)
    if len(cartas) == 2 and (s==9 or s==10 or s==11):
        puede = True
    return puede

def bj_puede_rendirse(cartas):
    res = False
    if len(cartas) == 2:
        res = True
    return res

def bj_evaluar_ganador(jugador_cartas, crupier_cartas):

    jugador_puntos = bj_suma_cartas(jugador_cartas)
    crupier_puntos = bj_suma_cartas(crupier_cartas)
    ...
    if jugador_puntos > crupier_puntos:
        proporcion = 1
        if bj_tiene_blackjack(jugador_cartas):
            proporcion = 3/2
    elif jugador_puntos < crupier_puntos:
        proporcion = -1
    else:
        if bj_tiene_blackjack(jugador_cartas) and bj_tiene_blackjack(crupier_cartas):
            proporcion = 0
        elif bj_tiene_blackjack(jugador_cartas):
            proporcion = 3/2
        elif bj_tiene_blackjack(crupier_cartas):
            proporcion = -1
    return proporcion
    
def blackjack(num_partida, fichas_apostadas):

    #Declarar variables
    partida_numero = num_partida
    ...
    def imprimir():
        ...
        print("Partida:", partida_numero, end="    -    ")
        if jugador_doblado:
        ...
        if len(jugador_cartas) != 0 and jugador_decidiendo == False:
            if bj_tiene_blackjack(jugador_cartas):
                print("Jugador Total: ", bj_suma_cartas(jugador_cartas), " BLACKJACK")
            else:
                print("Jugador Total: ", bj_suma_cartas(jugador_cartas))
        ...
        if len(crupier_cartas) != 0 and crupier_decidiendo == False:
            if bj_tiene_blackjack(crupier_cartas):
                print("Crupier Total: ", bj_suma_cartas(crupier_cartas), " BLACKJACK")
            else:
                print("Crupier Total: ", bj_suma_cartas(crupier_cartas))
        ...
        if jugador_decidiendo == True and mostrar_opciones == True:
            ...
            ganador = bj_evaluar_ganador(jugador_cartas, crupier_cartas)
            res_auxiliar = ""
            if ganador > 0:
    ...
    for i in range(2):
        jugador_cartas.append(sacar_carta_aleatoria(baraja))
    imprimir()
    ...
    # Si no hay blackjack automático
        ...
        s = bj_suma_cartas(jugador_cartas)
        opciones[1]= "Plantarse"
        opciones[2] = "Robar"
        # Poder Doblar
        if bj_puede_doblar(jugador_cartas):
            opciones[3] = "Doblar"
        # Poder Rendirse
        if bj_puede_rendirse(jugador_cartas):
            opciones[4] = "Rendirse"
    ...
    while(jugador_decidiendo):
        ...
        if decision == 1:
            ...
            if len(jugador_cartas) >= 2 and len(opciones) !=2:
                ...
                opciones[2] = "Robar"
                imprimir()
            if bj_suma_cartas(jugador_cartas) >= 21:
                mostrar_opciones = False
                imprimir()
                jugador_decidiendo = False
                imprimir()     
    ...
    for i in range(2):
        crupier_cartas.append(sacar_carta_aleatoria(baraja))
    imprimir()

    # Sacar más cartas si crupier necesita

    while (crupier_decidiendo):
        if bj_suma_cartas(crupier_cartas) >= 17:
    ...
    proporcion = bj_evaluar_ganador(jugador_cartas, crupier_cartas)
